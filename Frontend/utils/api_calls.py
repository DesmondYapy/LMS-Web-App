...
def get_instructor_course(role):
    response = requests.post(
        "http://localhost:8000/instructor-courses",
        json={"instructor_id": 0, "role": role},
    )
...
def get_overview_stats(instructor_courses):
    response = requests.post(
        "http://localhost:8000/overview-stats",
        json={
...
def get_course_stats(course_code):
    response = requests.post(
        "http://localhost:8000/course-stats",
        json={
...
def get_top_students(course_code):
    response = requests.post(
        "http://localhost:8000/top-students",
        json={
            "course_code": course_code,
        },
    )
...
def get_discussion_board(course_code):
    response = requests.post(
        "http://localhost:8000/discussion-board",
        json={
            "course_code": course_code,
        },
    )
...
def get_student_info(user_name):
    response = requests.post(
        "http://localhost:8000/student-search",
        json={
            "user_name": user_name,
        },
    )
    ...
    if response.status_code == 200:
        ...
        total_topics = (response.json()["total_topics"],)
        total_entries = (response.json()["total_entries"],)
        filtered_merged_df = (response.json()["filtered_merged_df"],)

        return user_id, total_topics, total_entries, filtered_merged_df

    elif response.status_code == 404:
        raise StudentNotFound(f"No user found with name {user_name}")
