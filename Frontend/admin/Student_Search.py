...
merged_entries_topic = topics.merge(entries, on="topic_id", how="left").merge(
...
with st.form("student-look-up-form"):
    ...
    if st.form_submit_button("Search"):
        ...
        if filtered.empty:
            ...
            merged_entries_topic["entry_created_at"] = pd.to_datetime(
                merged_entries_topic["entry_created_at"], errors="coerce"
            )
            merged_entries_topic = merged_entries_topic.dropna(
                subset=["entry_created_at"]
            )
            merged_entries_topic["week"] = (
                merged_entries_topic["entry_created_at"]
                .dt.to_period("W")
            ...
            for course in instructor_courses:
                with st.expander(f"{course}"):
                    ...
                    # Entries per topic
                    entries_per_topic = (
                        user_entries["topic_title"].value_counts().sort_index()
                    )
                    ...
                    weekly_topic_counts = (
                        merged_entries_topic[
                            (merged_entries_topic["course_code"] == course)
                            & (
                                merged_entries_topic["entry_posted_by_user_id"]
                                == user_id
                            )
                        ]
