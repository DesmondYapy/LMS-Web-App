...
def get_overview_stats(data: OverviewStatsRequest) -> OverviewStatsResponse:
    ...
    filtered_merged_t_c = merged_t_c[merged_t_c["course_code"].isin(instructor_courses)]
...
def get_at_risk_students(data: AtRiskRequest) -> AtRiskResponse:
    ...
    instructor_courses = data.instructor_courses
    course_ids = courses[courses["course_code"].isin(instructor_courses)][
        "course_id"
    ].tolist()
    ...
    # Extract stats
    at_risk_students = students_with_stats[students_with_stats["num_entries"] == 0][
