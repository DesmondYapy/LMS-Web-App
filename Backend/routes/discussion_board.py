...
def get_discussion_board(data: DiscussionBoardRequest) -> DiscussionBoardResponse:
    ...
    # Merge DataFrame
    merged_df = (
        topics.merge(entries, on="topic_id", how="left")
        .merge(courses, on="course_id", how="left")
        .merge(users, left_on="entry_posted_by_user_id", right_on="user_id", how="left")
    )
    ...
    grouped = filtered_df.sort_values("entry_created_at").groupby(
    ...
    for (topic_id, topic_title, topic_content), group_df in grouped:
        list_of_entries = (
            group_df[["entry_content", "entry_created_at", "user_name"]]
